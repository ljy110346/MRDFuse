...
# 读取数据
train_ir = torch.tensor(pd.read_excel('data/feiai/train/train_ir.xlsx', header=None).values, dtype=torch.float32)
train_raman = torch.tensor(pd.read_excel('data/feiai/train/train_raman.xlsx', header=None).values, dtype=torch.float32)
train_labels = torch.tensor(pd.read_excel('data/feiai/train/train_labels.xlsx', header=None).values.flatten(), dtype=torch.long)

val_ir = torch.tensor(pd.read_excel('data/feiai/val/val_ir.xlsx', header=None).values, dtype=torch.float32)
val_raman = torch.tensor(pd.read_excel('data/feiai/val/val_raman.xlsx', header=None).values, dtype=torch.float32)
val_labels = torch.tensor(pd.read_excel('data/feiai/val/val_labels.xlsx', header=None).values.flatten(), dtype=torch.long)
...
def train():
    ...
    torch.save(final_model_state, 'weights/model_feiai.pth')
    print('Final model saved to weights/model_feiai.pth')

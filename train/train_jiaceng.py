...
# 读取数据
train_ir = torch.tensor(pd.read_excel('data/jiaceng/train/train_ir.xlsx', header=None).values, dtype=torch.float32)
train_raman = torch.tensor(pd.read_excel('data/jiaceng/train/train_raman.xlsx', header=None).values, dtype=torch.float32)
train_labels = torch.tensor(pd.read_excel('data/jiaceng/train/train_labels.xlsx', header=None).values.flatten(), dtype=torch.long)

val_ir = torch.tensor(pd.read_excel('data/jiaceng/val/val_ir.xlsx', header=None).values, dtype=torch.float32)
val_raman = torch.tensor(pd.read_excel('data/jiaceng/val/val_raman.xlsx', header=None).values, dtype=torch.float32)
val_labels = torch.tensor(pd.read_excel('data/jiaceng/val/val_labels.xlsx', header=None).values.flatten(), dtype=torch.long)

# 封装数据集
batch_size = 32
train_dataset = TensorDataset(train_ir, train_raman, train_labels)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False)  # 不打乱数据
val_dataset = TensorDataset(val_ir, val_raman, val_labels)
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)  # 不打乱数据
...
def train():
    ...
    torch.save(final_model_state, 'weights/model_jiaceng.pth')
    print('Final model saved to weights/model_jiaceng.pth')
